...
class FeedForwardBatchMaskA2C:
    ...
    def train_one_episode(self):
        ...
        while not all(dones):
            # step the net
            action, policy, log_prob, entropy, value = self.net(
                obs, action_mask
            )
    ...
    def get_discounted_returns(self, rewards, values):
        ...
        # note: initial R should always be 0, either by masking or zero padding
        R = values[:, -1]
    ...
    def save_net(self, path):
        """
        Save the net given the whole path.
        """
