...
Recorded version: Compatibility version 1; checkpoint shape compatibility is distinguished by `network_type`
...
`global_shared` reuses the `node_shared` encoder and global hidden state. It adds `node_policy_context`, which combines each node embedding with the broadcast global hidden state through a shared ReLU layer before producing fixation logits. Existing `node_shared` params do not have this layer, so checkpoint shape compatibility is handled by `network_type` and parameter shape rather than by bumping `COMPAT_VERSION`.
