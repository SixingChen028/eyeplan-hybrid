...
from modules.parallel_ppo import PPOHyperParams, ParallelJaxBatchMaskPPO, ParallelPPOResult
...
def train_with_progress(
    trainer: ParallelJaxBatchMaskA2C | ParallelJaxBatchMaskPPO,
    ...
    *,
    ...
    output = (
        ...
        if algo == "a2c"
        else ParallelPPOResult(states=states, metrics=metrics),
        time.time() - start,
    )
    if include_gpu_summary:
...
def main() -> None:
    ...
    if int(fixed["t_max"]) != rollout_length:
        raise ValueError(
            f"rollout_length ({rollout_length}) must equal t_max ({fixed['t_max']}) for parallel training."
        )
    env = _env_from_args(combos[0])
    if algo == "a2c":
